...
Function OnProgramLoaded()
    ...
    On Error GoTo errHandler
    Call tl_pinListDataSort(False) 'Make the captured data = the pin list order!
    ' Put code here
    
    'Thehdw.Digital.Calibration.channels("14.).Excluded
    Exit Function
errHandler:
...
Function OnProgramValidated()
    ...
    ' Put code here
    'Power supplies to relays on.
    TheHdw.DIB.Power.Item("3.3V,5V_1,5V_2,12V,15V_1,15V_2").State = tlOn
    TheHdw.DCVI.EnableLevelSequence = True
...
Function OnProgramStarted()
    ...
    ' Put code here
    ' Before starting the program, set up default state for power supplies.
    ' Do not let IG-XL auto-range current.
    TheHdw.DCVS.ApplyLevelsAutoIrange = False
